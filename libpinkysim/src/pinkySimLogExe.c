...
void enableLogExe(const char* chipType)
{
	...
	snprintf(tmp_str, ARRAY_SIZE(tmp_str), "exeLog_%020llu.csv", 
		(uint64_t)rawtime);
	exeLogFile = fopen(tmp_str, "w");

	// Add first entry, which is header describing each column
	fprintf(exeLogFile, " Entry Num, ");
	fprintf(exeLogFile, "File Offset, ");
	fprintf(exeLogFile, "  Raw Data, ");
	...
	fprintf(exeLogFile, "%s, ", tmp_str);

	fprintf(exeLogFile, "  Step Num, ");

	fprintf(exeLogFile, "Current PC, ");
	fprintf(exeLogFile, "   Next PC, ");
	fprintf(exeLogFile, "        SP, ");
	fprintf(exeLogFile, "        LR, ");
	fprintf(exeLogFile, "      XPSR, ");
	fprintf(exeLogFile, "   PRIMASK, ");
	fprintf(exeLogFile, "   CONTROL, ");

	for (cnt = 0; cnt < 13; cnt++)	
	{
		fprintf(exeLogFile, "              Register % 2d, ", 
			(int)cnt);
	}
 ...
 * \return None.
 */
static void addLogExeEntry(const struct PinkySimContext* context, uint32_t offset, 
	uint32_t rawData, uint32_t size, const char* format, va_list arg)
{
	...
	fprintf(exeLogFile, "% 10d, ", entryNum);
	fprintf(exeLogFile, " 0x%08x, ", offset);
	switch(size)
	{
		case 1:
			fprintf(exeLogFile, "      0x%02x, ", 0xFF&rawData);
			break;
		case 2:
			fprintf(exeLogFile, "    0x%04x, ", 0xFFFF&rawData);
			break;
		case 4:
			fprintf(exeLogFile, "0x%08x, ", 0xFFFFFFFF&rawData);
			break;
		default:
			fprintf(exeLogFile, "Exception: invalidArgumentException for size %d", size);
			fflush(exeLogFile);
	...
	fprintf(exeLogFile, "%s, ", desc);

	fprintf(exeLogFile, "  % 8d, ", context->stepNum);

	fprintf(exeLogFile, "0x%08x, ", context->pc);
	fprintf(exeLogFile, "0x%08x, ", context->newPC);
	fprintf(exeLogFile, "0x%08x, ", context->spMain);
	fprintf(exeLogFile, "0x%08x, ", context->lr);
	fprintf(exeLogFile, "0x%08x, ", context->xPSR);
	fprintf(exeLogFile, "0x%08x, ", context->PRIMASK);
	fprintf(exeLogFile, "0x%08x, ", context->CONTROL);

	for (cnt = 0; cnt < ARRAY_SIZE(context->R); cnt++)	
	{
		fprintf(exeLogFile, "0x%08x (% 12d), ", context->R[cnt], context->R[cnt]);
	}
	...
	const char* format, ...)
{
	...
	addLogExeEntry(context, context->pc, instr, 2, format, args);
	...
	uint16_t instr1, uint16_t instr2, const char* format, ...)
{
	...
	addLogExeEntry(context, context->pc, instr, 4, format, args);

	va_end(args);
}

/**
 * Add log entry for memory access.
 *
 * \param[in] context Current processor context (reg values, etc.)
 * \param address Address read from or written to.
 * \param result Value read from or written to address.
 * \param size Number of valid bytes in result.
 * \param[in] format Human readable description of instruction.
 * \param ... Additional argument like for printf().
 *
 * \return None.
	...
	uint32_t result, uint32_t size, const char* format, ...)
{
	...
	char new_format[MAX_DESC_STR_LEN];
	strncpy(new_format, format, MAX_DESC_STR_LEN);
	new_format[MAX_DESC_STR_LEN-1] = 0;
	strncat(new_format, getMemInfo(address), MAX_DESC_STR_LEN);
	new_format[MAX_DESC_STR_LEN-1] = 0;

	addLogExeEntry(context, address, result, size, new_format, args);
