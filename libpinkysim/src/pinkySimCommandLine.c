...
static void displayUsage(void)
{
    printf("Usage: pinkySim [--ram baseAddress size] [--flash baseAddress size] [--gdbPort tcpPortNumber]\n"
           ...
           "                [--logExe chipType] imageFilename.bin [args]\n"
           ...
           "       --logExe specifies that a csv formatted log should be created for each instruction and memory access\n"
...
static int parseLogExeOption(pinkySimCommandLine* pThis, int argc, const char** ppArgs);
...
static int parseFlagArgument(pinkySimCommandLine* pThis, int argc, const char** ppArgs)
{
    ...
    else if (0 == strcasecmp(*ppArgs, "--logExe"))
        return parseLogExeOption(pThis, argc - 1, &ppArgs[1]);
    else
