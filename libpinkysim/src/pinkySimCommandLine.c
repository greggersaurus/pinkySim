...
static void displayUsage(void)
{
    printf("Usage: pinkySim [--ram baseAddress size] [--flash baseAddress size] [--gdbPort tcpPortNumber]\n"
           "                [--breakOnStart] [--codecov application.elf resultsDirectory] [--restrict sourcePathPrefix]\n"
           "                [--log chipType] imageFilename.bin [args]\n"
           ...
           "       --log specifiies that a csv formatted log should be created for each instruction and memory access\n"
           "         executed. chipType is used to specify expected memory region layout (i.e. control register\n"
           "         designations).\n"
...
static int parseCodeCovOption(pinkySimCommandLine* pThis, int argc, const char** ppArgs);
static int parseRestrictOption(pinkySimCommandLine* pThis, int argc, const char** ppArgs);
static int parseLogOption(pinkySimCommandLine* pThis, int argc, const char** ppArgs);
...
static int parseFlagArgument(pinkySimCommandLine* pThis, int argc, const char** ppArgs)
{
    ...
    else if (0 == strcasecmp(*ppArgs, "--codecov"))
        return parseCodeCovOption(pThis, argc - 1, &ppArgs[1]);
    else if (0 == strcasecmp(*ppArgs, "--restrict"))
        return parseRestrictOption(pThis, argc - 1, &ppArgs[1]);
    else if (0 == strcasecmp(*ppArgs, "--log"))
        return parseLogOption(pThis, argc - 1, &ppArgs[1]);
    else
...
static int parseCodeCovOption(pinkySimCommandLine* pThis, int argc, const char** ppArgs)
{
    ...
    pThis->pCoverageElfFilename = ppArgs[0];
    pThis->pCoverageResultsDirectory = ppArgs[1];
    return 3;
}

static int parseRestrictOption(pinkySimCommandLine* pThis, int argc, const char** ppArgs)
{
    uint32_t      pathCount = pThis->coverageRestrictPathCount + 1;
    const char**  pRealloc;
    ...
    pRealloc = realloc(pThis->ppCoverageRestrictPaths, sizeof(*pRealloc) * pathCount);
    if (!pRealloc)
        __throw(outOfMemoryException);
    pThis->ppCoverageRestrictPaths = pRealloc;
    pThis->coverageRestrictPathCount = pathCount;
    pThis->ppCoverageRestrictPaths[pathCount - 1] = ppArgs[0];
...
static int parseLogOption(pinkySimCommandLine* pThis, int argc, const char** ppArgs)
{
    ...
    pThis->logChipType = ppArgs[0];

    enableLog(pThis->logChipType);
...
{
    MemorySim_Uninit(pThis->pMemory);
    free(pThis->ppCoverageRestrictPaths);
}
